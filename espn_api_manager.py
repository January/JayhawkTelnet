...
class ESPNAPIManager:
    ...
    # Returns the men's CBB schedule as a list
    def MensCBBSchedule():
    ...
    # Grabs women's CBB stats from ESPN and returns them as a dict
    def UpdateWomensCBBStats():
        team = requests.get(f'https://site.api.espn.com/apis/site/v2/sports/basketball/womens-college-basketball/teams/{team_id}').json()
        schedule = requests.get(f'https://site.api.espn.com/apis/site/v2/sports/basketball/womens-college-basketball/teams/{team_id}/schedule').json()
        win = int(team['team']['record']['items'][0]['stats'][18]['value'])
        loss = int(team['team']['record']['items'][0]['stats'][9]['value'])
        conf_win = ""
        conf_loss = ""
    ...
    # Returns the men's CBB schedule as a list
    def WomensCBBSchedule():
        endpoint = f"https://site.api.espn.com/apis/site/v2/sports/basketball/womens-college-basketball/teams/{team_id}/schedule"
        return ESPNAPIManager.GetSchedule(endpoint, "women's basketball")